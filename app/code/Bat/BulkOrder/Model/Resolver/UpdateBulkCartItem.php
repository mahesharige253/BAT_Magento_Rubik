...
class UpdateBulkCartItem implements ResolverInterface
{
    /**
     * @var CartDetails
     */
    ...
    public function resolve(Field $field, $context, ResolveInfo $info, array $value = null, array $args = null)
    {
        ...
        if (empty($args['input']['quantity'])) {
            throw new GraphQlInputException(__('"Cart item quantity" value should be specified'));
        }
        ...
        $productSku = $this->cartDetails->getProductSkuFromCartItemId($cartItemId, $maskedCartId);
        if ($this->cartDetails->updateQuoteItem($maskedCartId, $cartItemId, $quantity)) {
            $discountMessage = [];
            $itemDiscountMessage = '';
            ...
            if (!empty($productMatched)) {
                foreach ($productMatched as $productCondition) {
                    if ($productSku == $productCondition) {
                        $itemDiscountMessage = $itemdiscountMessage[0][$productSku];
                    }
                }
