...
AdminRma.prototype = {
    ...
    addOrderItemToGrid: function (orderItem, className, hasUserAttributes) {
        ...
        if(!this.orderItemArray.includes(orderItem['item_id'])){
            this.orderItemArray.push(orderItem['item_id']);
        } else {
            return;
        }

        var row = new Element('tr', {id: 'id_' + newRmaItemId, 'class': className ? 'even' : 'odd', order_item_id: orderItem['item_id']});
    ...
    deleteRow: function (event) {
        ...
        var deleteItem = event.findElement('a').up(1);
        var ItemIdForDelete = deleteItem.getAttribute('order_item_id');
        let found = this.orderItemArray.find(function (element) {
            return element == ItemIdForDelete;
        });
        if(found != undefined){
            this.orderItemArray.splice(this.orderItemArray.indexOf(ItemIdForDelete),1);
        }
